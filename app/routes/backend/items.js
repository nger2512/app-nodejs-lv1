...
const itemModel = require(__path_models+'items');
...
router.post('/change-status/:status/',async(req,res)=>{
    ...
    await itemModel.changeStatus(req.body.cid,currentStatus,{task:'update-many'}).then((result,err)=>{ // affected is array
...
router.post('/change-ordering',async(req,res)=>{
    ...
    itemModel.changeOrdering(cids,orderings,null).then((result)=>{  
...
router.get('/delete/:id',async(req,res)=>{
    let id = paramsHelpers.getParams(req.params,'id','');
    await itemModel.deleteItem(id,{task:'delete-one'})
    .then((result)=>{
...
router.post('/delete',async(req,res)=>{
    await itemModel.deleteItem(req.body.cid,{task:'delete-many'}).then((result,err)=>{ // affected is array
        if(err) console.log(err);
        console.log(result);
...
router.get('/form(/:id)?',async(req,res)=>{
    ...
    if(id !=='' &&  id !== undefined){
        itemModel.getItem(id).then((item,err)=>{
